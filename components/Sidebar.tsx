...
const Sidebar: React.FC<SidebarProps> = ({ features, activeFeature, onSelectFeature, isOpen, setIsOpen }) => {
  return (
    <nav className={`fixed inset-y-0 left-0 w-72 bg-slate-950/70 backdrop-blur-xl border-r border-slate-800/80 p-4 flex flex-col h-screen z-30 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:flex-shrink-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <li key={feature.id}>
              <button
                ...
                className={`w-full flex items-center text-left p-3 rounded-lg transition-all duration-200 ${
                  isActive
                    ? 'bg-cyan-900/50 text-white border-l-2 border-cyan-400 sidebar-active-glow'
                    : 'border-l-2 border-transparent text-slate-400 hover:bg-slate-800/60 hover:text-slate-200 hover:border-slate-700'
                }`}
              >
